...
function parseDocFile(file, clients) {
  ...
  for (const c of clients) {
    const fileNo = (c.ceilao_ib_file_no || '').toString().replace(/[^a-zA-Z0-9_-]/g, '_');
    if (!fileNo) continue;
...
const TableSection = () => {
  ...
  const runDocImport = async () => {
    ...
    for (const item of toUpload) {
      ...
      try {
        const url = await uploadToCloudinary(item.file, 'ceilao/docs-restore');
                  ...
                  <Box key={i} sx={{ px: 2, py: 1.2, display: 'flex', alignItems: 'center', gap: 1.5,
                    ...
                    <Box sx={{ flex: 1, minWidth: 0 }}>
                      ...
                      <Typography sx={{ fontSize: 11, color: '#9CA3AF' }}>
                        {item.client.client_name} · {item.client.ceilao_ib_file_no} · <strong style={{ color: '#6366f1' }}>{item.docLabel}</strong>
